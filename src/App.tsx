...
interface ToolCategoryWithSub {
  title: string;
  subcategories?: Record<string, ToolSubCategory>;
  tools?: ToolItem[];
}

const initialToolCategories: Record<string, ToolCategoryWithSub> = {
  tools: {
    title: '在线工具',
    subcategories: {
      ai: {
      ...
      common: {
        title: '常用工具',
         ...
         { name: 'Gitee', desc: '国内代码托管平台', icon: Code, url: 'https://gitee.com/explore', count: 0 },        
          ...
          { name: '135编辑器', desc: '公众号排版', icon: Tool, url: 'https://www.135editor.com', count: 0 },
          ...
          { name: 'TOolfk工具网', desc: '在线工具合集', icon: Tool, url: 'https://www.toolfk.com', count: 0 },
          ...
          { name: '奔跑中的奶酪', desc: '高效工具方法合集', icon: Tool, url: 'https://runningcheese.com', count: 0 },
          { name: '帮小忙工具箱', desc: 'QQ 浏览器在线工具箱', icon: Tool, url: 'https://tool.browser.qq.com/', count: 0 },
     ...
     { name: 'AGE动漫', desc: '动漫网站合集', icon: FileVideo, url: 'https://www.agedm.org/', count: 0 },
      ...
      study:{
        title:'学习相关',
          ...
          { name: '运营笔记导航', desc: '运营学习导航', icon: Tool, url: 'https://yunyingbiji.cn', count: 0 },
...
{ name: '极像素', desc: '超高清全景图库', icon: Building2, url: 'https://www.sigoo.com/', count: 0 },
      ...
      { name: '网址导航「你不会百度吗」', desc: '懒人问答网站', icon: Tool, url: 'https://buhuibaidu.me', count: 0 },
...
const faqs = [
  {
    question: '如何提交新工具？',
    answer:
      '您可以通过页面底部的联系方式向我们提交新工具建议。我们会在审核后及时添加。',
  },
  {
    question: '收录标准是什么？',
    answer:
      '我们主要考虑工具的实用性、可靠性和用户评价。确保每个收录的工具都能真正帮助到用户。',
  },
];
...
function App() {
  ...
  const [toolCategories, setToolCategories] = useState(() => {
    ...
    if (savedCounts) {
      ...
      const categoriesWithCounts = { ...initialToolCategories };
      
      Object.keys(categoriesWithCounts).forEach(categoryKey => {
        const category = categoriesWithCounts[categoryKey];
        if (category.subcategories) {
          Object.keys(category.subcategories).forEach(subKey => {
            category.subcategories![subKey].tools = category.subcategories![subKey].tools.map(tool => ({
              ...tool,
              count: counts[tool.url] || 0
            }));
            category.subcategories![subKey].tools.sort((a, b) => b.count - a.count);
          });
          ...
          category.tools.sort((a, b) => b.count - a.count);
        }
      });
  ...
  const handleToolClick = (categoryKey: string, toolIndex: number) => {
    const newCategories = { ...toolCategories };
    let tool: ToolItem;
    
    if (newCategories[categoryKey].subcategories && activeSubcategory[categoryKey]) {
      tool = newCategories[categoryKey].subcategories![activeSubcategory[categoryKey]].tools[toolIndex];
    } else {
      tool = newCategories[categoryKey].tools![toolIndex];
    }
    
    tool.count += 1;
    
    if (newCategories[categoryKey].subcategories && activeSubcategory[categoryKey]) {
      newCategories[categoryKey].subcategories![activeSubcategory[categoryKey]].tools.sort((a, b) => b.count - a.count);
    } else {
      newCategories[categoryKey].tools!.sort((a, b) => b.count - a.count);
    }
    
    setToolCategories(newCategories);
    
    const counts = Object.keys(newCategories).reduce((acc, key) => {
      const category = newCategories[key];
      if (category.subcategories) {
        Object.keys(category.subcategories).forEach(subKey => {
          category.subcategories![subKey].tools.forEach(tool => {
            acc[tool.url] = tool.count;
          });
        });
      } else if (category.tools) {
              ...
              {(category.subcategories && activeSubcategory[key] 
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <ExternalLink className="h-4 w-4 text-gray-400 dark:text-gray-500 opacity-0 group-hover:opacity-100 transition-opacity duration-200" />
                  </div>
        ...
        <div className="max-w-3xl mx-auto">
          {faqs.map((faq, index) => (
            <div key={index} className="mb-4">
              ...
              {activeFaq === index && (
                <div className="bg-white dark:bg-gray-800 px-4 py-3 rounded-b-lg shadow-md mt-1">
